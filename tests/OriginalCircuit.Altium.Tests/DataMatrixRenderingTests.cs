...
/// <summary>
/// Integration tests for rendering a Data Matrix barcode (PCB <c>Text</c> with
/// <see cref="PcbBarCodeKind.DataMatrix"/>). Mirrors the Coherent Digitiser: an inverted symbol on the top
/// solder-mask layer over a copper backing fill, which should read as a gold field with green data modules.
/// </summary>
public sealed class DataMatrixRenderingTests
{
    ...
    private static PcbDocument BuildBoard(bool withBarcode)
    {
        ...
        if (withBarcode)
        {
            board.AddText(new PcbText
            {
                ...
                BarCodeType = PcbBarCodeKind.DataMatrix,
    ...
    private static async Task<int> CountGoldAsync(PcbDocument board)
    {
    ...
    [Fact]
    public async Task InvertedDataMatrix_OnSolderMask_RevealsGoldField()
    {
        // The same board with the barcode removed leaves the copper fully masked (no gold). Adding the
        // inverted Data Matrix opens the mask over its field, exposing a large gold area.
        int goldWithout = await CountGoldAsync(BuildBoard(withBarcode: false));
        int goldWith = await CountGoldAsync(BuildBoard(withBarcode: true));

        Assert.True(goldWith > goldWithout + 1000,
            $"Inverted Data Matrix should expose gold finish (with={goldWith}, without={goldWithout}).");
    }

    [Fact]
    public async Task DataMatrix_LeavesDataModulesMasked()
    {
        // Inverted symbol: the field is gold but the dark data modules stay masked (green), so the gold area
        // is well short of the whole 7.5mm box. Confirm the symbol isn't a solid gold square.
        var renderer = new RasterRenderer();
        using var ms = new MemoryStream();
        await renderer.RenderRealisticAsync(BuildBoard(withBarcode: true), ms,
            new RenderOptions { Width = 500, Height = 500 }, PcbRealisticStyle.GreenEnig.For(PcbViewSide.Top));
        ms.Position = 0;
        using var bitmap = SKBitmap.Decode(ms);

        int gold = 0, maskGreen = 0;
        for (int y = 0; y < bitmap.Height; y++)
            for (int x = 0; x < bitmap.Width; x++)
            {
                var p = bitmap.GetPixel(x, y);
                if (IsGold(p.Red, p.Green, p.Blue)) gold++;
                else if (p.Green > p.Red && p.Green > p.Blue) maskGreen++;
            }

        Assert.True(gold > 0, "barcode field should expose some gold");
        Assert.True(maskGreen > 0, "data modules (and board) should remain green-masked");
    }
}
